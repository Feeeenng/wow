...
- 安装包预计增加约 25 到 30 MB，安装后媒体组件占用约 54 MB；用户不需要额外下载。仍需在重新启动的 `tauri dev` 中复核动态画面、音轨、停止直播和客户端退出清理。
- 本轮利用用户已有的 `tauri dev` 热更新完成真实 OBS、MediaMTX、WHIP 和 WHEP 联调；未主动运行前端构建、Rust 检查、测试或打包。
