...
- 客户端正常退出时同步关闭由自身启动的 OBS：先请求 Windows 正常关闭并等待，超时后才强制结束；退出阶段停止 OBS 守护，避免重新拉起或触发 OBS 异常关闭恢复提示。
- OBS 专属场景固定维护“魔兽世界游戏画面”游戏捕捉来源，使用窗口模式绑定 `Wow.exe` 并按 1920×1080 等比填充；仅当 OBS 实时枚举到 WoW 窗口且来源绑定正常时，前端和 Rust 后端才允许开始测试录制。
- OBS 设置选择后立即生效，不设置独立保存按钮。编码器读取 OBS profile；声音只包含扬声器和麦克风两个监听设备，可选来源读取 OBS 输入源属性；用户界面不展示 WebSocket 地址、协议版本、鉴权和便携模式等技术信息。
