...
- 客户端正常退出时同步关闭由自身启动的 OBS：先请求 Windows 正常关闭并等待，超时后才强制结束；退出阶段停止 OBS 守护。便携 OBS 新进程启动前清理自身 `.sentinel` 异常退出标记；受管进程长时间无法提供 WebSocket 时结束并在下一轮重启，避免安全模式对话框持续阻塞。
- OBS 专属场景固定维护“魔兽世界游戏画面”来源并按 1920×1080 等比填充；捕捉类型、自动捕捉和目标窗口从 OBS 实际属性读取并由 Rust 写回。仅当 `Wow.exe` 真实运行、OBS 实时枚举到 WoW 窗口且来源绑定正常时，前端和 Rust 后端才允许开始测试录制。
