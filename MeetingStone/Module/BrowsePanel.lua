...
function BrowsePanel:OnInitialize()
  ...
  self.filters = {}
  self.IgnoreLeaderOnly = {}
  self.IgnoreWithTitle = {}
  self.IgnoreWithLeader = {}
  ...
  do
    ...
    ActivityList:RegisterFilter(function(activity, ...)
      ...
      if self.IgnoreLeaderOnly[leader] then
        local ist = true
        ...
        if ist then
          table.insert(Profile.gdb.global.UIMemory.IGNORE_LIST, 1, {
            leader = leader,
      ...
      if Profile:GetEnableIgnoreTitle() then
        ...
        if self.IgnoreWithTitle[title] then
          if not self.IgnoreWithLeader[leader] then
            self.IgnoreWithLeader[leader] = true
            table.insert(Profile.gdb.global.UIMemory.IGNORE_LIST, 1, {
              leader = leader,
              time = date('%Y-%m-%d %H:%M', time()),
              dep = '由指定标题传染屏蔽',
      ...
      if self.IgnoreWithLeader[leader] then
        return false
      end

      if Profile.gdb.global.UIMemory['SCORE'] then
        if not activity:GetLeaderScore() or activity:GetLeaderScore() < Profile.gdb.global.UIMemory['SCORE'] then
          return false
        end
      end

      if BrowsePanel.ActivityDropdown:GetText() == activitytypeText1 and Profile.cdb.profile.dungeonFilters then
        if not Profile.cdb.profile.dungeonFilters[activity:GetName()] then
          return false
        end
      end

      if BrowsePanel.ActivityDropdown:GetText() == activitytypeText1 and Profile.cdb.profile.roleFilters then
        if Profile.cdb.profile.roleFilters.haveHealer and data.HEALER <= 0 then
          return false
        end
        if Profile.cdb.profile.roleFilters.noHealer and data.HEALER > 0 then
          return false
        end
        if Profile.cdb.profile.roleFilters.haveTank and data.TANK <= 0 then
          return false
        end
        if Profile.cdb.profile.roleFilters.noTank and data.TANK > 0 then
          return false
        end
        if Profile.cdb.profile.roleFilters.noDps and data.DAMAGER > 2 then
          return false
        end
        local totalCount = (data.TANK or 0) + (data.HEALER or 0) + (data.DAMAGER or 0)
        if Profile.cdb.profile.roleFilters.vacancyChecked and Profile.cdb.profile.roleFilters.vacancyMinimumRating ~= (5 - totalCount) then
          return false
        end
        local score = activity:GetLeaderScore()
        if Profile.cdb.profile.roleFilters.secretNumberChecked and score < Profile.cdb.profile.roleFilters.secretNumberMinimumRating then
          return false
        end
      end

      local classFilter = Profile.gdb.global.UIMemory.ClassNeed == false
      ...
      for i = 1, activity:GetNumMembers() do
        ...
        if Profile.gdb.global.UIMemory[class] == true then
          if Profile.gdb.global.UIMemory.ClassNeed then
            classFilter = true
          else
            classFilter = false
          end
      ...
      if classFilter == false then
        for classID = 1, GetNumClasses() do
          ...
          if Profile.gdb.global.UIMemory[classFile] == true then
            allnoCheck = false
          end
        end
      end
    ...
    ActivityList:InitHeader {
      ...
      --    end
      --    local regimentData = Profile.cdb.realm.Locomotive.LocomotiveData[Leader]
      --    if not regimentData or regimentData.level == nil or regimentData.level == -1 then
      --      return
      --    end
      --    return format([[Interface\AddOns\MeetingStone\Media\Locomotive\Locomotive%d]], regimentData.level), 0, 0.75, 0,0.75
      --  end
      --},
      {
        key = '@',
      ...
      --    end
      --    local regimentData = Profile.cdb.realm.Locomotive.LocomotiveData[Leader]
      --    if not regimentData or regimentData.roomID == nil then
      --      return
      --    end
      --    return [[Interface\AddOns\MeetingStone\Media\buttonInRoom]]
      --  end
      --},
      {
        key = 'LeaderScore',
    ...
    ActivityList:SetCallback('OnGridClick_operate', function(_, button, activity)
    ...
    ActivityList:SetCallback('OnItemFormatted', function(_, button, item)
      ...
      if not button.SameInstanceBgLeft then

        local iconFrame = CreateFrame("Frame", nil, button)
        iconFrame:SetSize(968, 32)
        iconFrame:SetPoint("CENTER")
        button.SameInstanceBgLeft = iconFrame:CreateTexture(nil, "ARTWORK")
        button.SameInstanceBgLeft:SetAllPoints()
        button.SameInstanceBgLeft:SetTexCoord(0, 0.5253, 0, 0.5625)
      end
      if regimentData and regimentData.bgID ~= nil and regimentData.bgID ~= 0 then
        local activityInfo = C_LFGList.GetActivityInfoTable(item:GetActivityID())
        if regimentData.bgID == 100 and activityInfo and activityInfo.displayType ~= Enum.LFGListDisplayType.RoleEnumerate then
          button.SameInstanceBgLeft:Hide()
        else
          button.SameInstanceBgLeft:SetTexture("Interface/AddOns/MeetingStone/Media/ProgressBg/ProgressBg" .. regimentData.bgID)
          button.SameInstanceBgLeft:Show()
        end
      else
        button.SameInstanceBgLeft:Hide()
      end
  ...
  do
    ...
    SignUpButton:SetScript('OnClick', function()
      ...
      LogStatistics:InsertLog({ time(), 1, self.ActivityList:GetSelectedItem(), GetActivityCategoryName(self.ActivityList:GetSelectedItem()) })
    end)
  ...
  do
    ...
    ActivityDropdown:SetCallback('OnSelectChanged', function(_, data, ...)
      if DealBrowsePanel ~= nil and (not DealBrowsePanel.ActivityDropdown:GetItem() or DealBrowsePanel.ActivityDropdown:GetItem().value ~= data.value) then
        DealBrowsePanel.ActivityDropdown:SetItem(data)
      end
      -- Modification begin
      ...
      if not self:InSet() then
        if data.activityId then
          ...
          if DealBrowsePanel ~= nil and DealBrowsePanel.ActivityDropdown and data.value ~= DealBrowsePanel.ActivityDropdown:GetValue() then
            DealBrowsePanel.ActivityDropdown:SetValue(data.value)
          end
          self:EndSet()
        else
  ...
  local AdvFilterPanel = CreateFrame('Frame', nil, MainPanel, 'SimplePanelTemplate')
  do
  ...
  do
    ...
    quickJoinCheckBox:SetScript("OnClick", function(self, event, arg1)
      ...
      if not Profile.gdb.global.UIMemory then
        Profile.gdb.global.UIMemory = {}
      end
      Profile.gdb.global.UIMemory.QuickJoin = Private_EnableQuickJoin
    end)
  ...
  if Profile.gdb.global.UIMemory and (Profile.gdb.global.UIMemory.QuickJoin ~= nil) then
    quickJoinCheckBox:SetChecked(Profile.gdb.global.UIMemory.QuickJoin)
    Private_EnableQuickJoin = Profile.gdb.global.UIMemory.QuickJoin
  end
...
function BrowsePanel:DoSearch()
  ...
  self.ResetFilterButton:Disable()
  self.SearchFilterButton:Disable()
  if self.MD and #self.MD ~= 0 then
    for i, box in ipairs(self.MD) do
      box.Check:Disable()
    end
  end
  self:Search()
  Logic:SendServerCQTL()
  Logic:SendServerCQGLIB()
...
function BrowsePanel:OnRefreshTimer()
  ...
  self.ResetFilterButton:Enable()
  self.SearchFilterButton:Enable()
  if self.MD and #self.MD ~= 0 then
    for i, box in ipairs(self.MD) do
      box.Check:Enable()
    end
...
-- 进语音房间
function BrowsePanel:OpenVoiceRoom(activity)
  local Leader = activity:GetLeader()
  if not Leader then
    return
  end
  ...
  if regimentData then
    ...
    _G.ThreeDimensionsCode:sendCommand('joinRoom', TableToJson(sendData))
    _G.isOpenVoiceRoom = false

    GameTooltip:SetOwner(self, 'ANCHOR_CURSOR')
    GameTooltip:SetText(L['正在进入房间中...'], 1.0, 1.0, 1.0)
    GameTooltip:Show()

    local data = _G.Base64:EnCode(format('%s+%d', UnitGUID("player"), GetRealmID()))
    LogStatistics:InsertLog({ time(), 4, 1, data })

    C_Timer.After(3, function(...)
      GameTooltip:Hide()
      if _G.isOpenVoiceRoom then
        return
      end
      _G.ThreeDimensionsCode:sendCommand('joinRoom', '-1')
      LogStatistics:InsertLog({ time(), 4, 2, data })
      GUI:CallUrlDialog(format('https://dd.163.com/room/%s/?utm_source=jjh&utm_term=%s', currentRoomID, data))
    end)
  end
end
