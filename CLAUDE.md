...
- 直播页本地预览固定使用 OBS Virtual Camera：Rust 通过 `obws` 校验专属场景并启停虚拟摄像头输出，React 只申请系统返回的 `OBS Virtual Camera` 视频轨并挂载到原生 `video`。便携 OBS 安装阶段通过一次 Windows 管理员授权注册官方 32/64 位组件。本地预览不再启动 SRT、FFmpeg 或 HLS 服务；内置 FFmpeg 保留给后续录像重封装、切片等媒体处理。未来云端直播使用独立编码传输入口和服务端播放地址，不复用系统虚拟摄像头，也不允许 React 直接控制 OBS 或云端推流协议。
